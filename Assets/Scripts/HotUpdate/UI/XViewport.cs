    ...
    public class XViewport : MonoBehaviour
    {
        ...
        public string activeView
        {
            ...
            set
            {
                ...
                //XLogger.DEBUG_Format("set asset");
        ...
        IEnumerator LoadViewCoroutine()
        {
            ...
            {
                if (AssetManagement.AssetCache.ContainsRawObject(assetName))
                {
                    GameObject go = AssetManagement.AssetCache.InstantiateObject<GameObject>(assetName, m_Transform);
                ...
                {
                    AssetManagement.AssetInternalLoader async = AssetManagement.AssetUtility.LoadAsset<GameObject>(assetName);
                    ...
                    ViewLoader vloader = m_Transform.TryGetComponent<ViewLoader>();
                    vloader.loader = async;
                    vloader.ReStart();

                    //XLogger.DEBUG_Format("start load");
                    async.Update();
                    yield return async;
                    //XLogger.DEBUG_Format("end load");
        ...
        public void ResetActiveName()
        {
            m_ActiveView = string.Empty;
        }
        ...
        void SelectChange()
        {
            ...
            else
            {
                ...
                if (active != null && active.GameObject)
                {
                    if (active.GameObject.GetComponent<Animator>())
                        active.RectTransform.localPosition = new Vector3(-5000, -5000, 0);
                    else
                    {
                        if (activeType == ActiveType.Active)
                            active.RectTransform.localPosition = Vector3.zero;
                    ...
                    active.CanvasGroup.DOFade(1, this.m_TweenTime).OnComplete(() =>
                    {
                        ...
                        if (waitComplete)
                        {
                            ViewportInfo curView;
                            //activeView可能在动画结束前被修改
                            if (m_Views.TryGetValue(activeView, out curView))
                            {
                                if (curView.GameObject != active.GameObject)
                                {
                                    return;
                                }
                            }
        ...
        void OnDestroy()
        {
            if (this.m_Views != null)
            {
                foreach (var view in this.m_Views)
                {
                    ...
                    {
                        if (m_LoadType == LoadType.AssetManagement)
                            AssetManagement.AssetUtility.DestroyAsset(view.Value.GameObject);
                        else
