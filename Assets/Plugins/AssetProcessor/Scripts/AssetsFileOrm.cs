...
#pragma warning disable 0414
    ...
    public void RepeCheck()
    {
        ...
        foreach (var item in m_AllFileOrm)
        {
            ...
            {
                if (dic.ContainsKey(info.p_AssetHashBundleName))
                {
                    FileOrm.AssetBundleInfo exist = dic[info.p_AssetHashBundleName];
                    log(string.Format("重复资源包 AssetBundleName [{0}]:[{1}]   [{2}]:[{3}]", exist.p_AssetBundleName, exist.p_AssetHashBundleName, info.p_AssetBundleName, info.p_AssetHashBundleName));
                }
                else
                {
                    dic.Add(info.p_AssetHashBundleName, info);
                }
...
#if UNITY_EDITOR
    public static AssetsFileOrm.FileOrm CreateFileOrm(string path, List<UnityEditor.AssetBundleBuild> obuilds, UnityEditor.AssetBundleBuild[] builds, List<string> hashs)
    {
        FileOrm fileOrm = new FileOrm(path);
        ...
        for (int j = 0; j < builds.Length; j++)
        {
            ...
            string abPath = Path.Combine(parent, info.p_AssetHashBundleName);
