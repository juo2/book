...
public class ReferenceFinderData
{
    ...
    //反引用
    public void GetNoRefs(string storage)
    {
        if (!Directory.Exists(storage))
            return;

        CSVReport.Init(storage, "反引用");

        HashSet<string> ExtensionMap = new HashSet<string>()
        {
            ".fbx",
            ".fbc",
            ".png",
        ...
        try
        {
            foreach (var asset in assetDict)
            {
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("Processing..", asset.Value.path, 1))
                    break;

                var ext = Path.GetExtension(asset.Value.path).ToLower();
                //指定特定后缀和文件夹类型
                if (!ExtensionMap.Contains(ext))
                {
                    if (asset.Value.path.IndexOf(@"/Effect/") < 0)
                        continue;
                }

                if (asset.Value.path.IndexOf(@"Assets/Shaders/") >= 0)
                    continue;

                if (asset.Value.references.Count <= 0)
                {
                    var line = new CSVLine(asset.Value.path);
                    line.ApplyLine();
                }
            }
        }
    ...
    //乱引用
    public void GetMassRefs(string storage)
    {
        if (!Directory.Exists(storage))
            return;

        CSVReport.Init(storage, "乱引用");
        CSVReport.csvStr = @"路径,文件夹类型,文件夹名,类型,依赖关系,文件夹外引用数(除Assets/Shaders/)";
        ...
        try
        {
            foreach (var asset in assetDict)
            {
                ...
                {
                    var path = AssetDatabase.GUIDToAssetPath(dependency);
                    //shader除外
                    if (path.IndexOf(@"Assets/Shaders/") >= 0)
                        continue;
                    //MatCap除外
                    if (path.IndexOf(@"Assets/Art/Charactars/MatCap") >= 0)
                        continue;

                    var folder = CSVLine.GetFolderName(path);
                ...
                if (isMess)
                {
                    var line_main = new CSVLine(asset.Value.path);
                    line_main.SetType(ext);
                    line_main.SetSize("依赖主");
                    line_main.SetCompress(folderNames.Count.ToString());
                    line_main.ApplyLine();
                    ...
                    foreach (var dependency in asset.Value.dependencies)
                    {
                        ...
                        line.SetPath("L " + path);
                        line.SetType(Path.GetExtension(path).ToLower());
                        line.SetSize("依赖");
                        line.ApplyLine();
                    }
                }
    ...
    public void GetBeRef(string storage)
    {
        if (!Directory.Exists(storage))
            return;

        CSVReport.Init(storage, "乱引用");
        CSVReport.csvStr = @"路径,文件夹类型,文件夹名,类型,被引用次数";
