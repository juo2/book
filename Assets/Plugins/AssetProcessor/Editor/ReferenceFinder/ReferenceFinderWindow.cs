...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (m_AssetTreeView != null)
        {
            float ext = extra ? toolbarGUIStyle.fixedHeight * 2.5f : 0.0f;
            //绘制Treeview
            m_AssetTreeView.OnGUI(new Rect(0, toolbarGUIStyle.fixedHeight + ext, position.width, position.height - toolbarGUIStyle.fixedHeight));
        }
    ...
    public void DrawOptionBar()
    {
        ...
        //额外工具
        bool IsExtra = extra;
        IsExtra = GUILayout.Toggle(extra, extra ? "ExtraTool(On)" : "ExtraTool(Off)", toolbarButtonGUIStyle, GUILayout.Width(100));
        if (IsExtra != extra)
        {
            extra = IsExtra;
        }
    ...
    public static string noRefPathPrefabKey = "__noRefPathPrefabKey";
    public static string noRefPath = string.Empty;

    public static string messPathPrefabKey = "__messPathPrefabKey";
    public static string messPath = string.Empty;

    bool extra = true;
    //额外工具
    public void DrawExtraTool()
    {
        if (!extra)
            return;

        noRefPath = EditorPrefs.GetString(noRefPathPrefabKey);
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(noRefPath, (GUIStyle)"HelpBox");
            if (GUILayout.Button("报告导出路径"))
            {
                var res = EditorUtility.OpenFolderPanel("选择报告导出路径", noRefPath, noRefPath);
                if (string.IsNullOrEmpty(res))
                {
                    return;
                }
                noRefPath = res;
                EditorPrefs.SetString(noRefPathPrefabKey, noRefPath);
                Repaint();
                return;
            }

            if (GUILayout.Button("未被引用Assets"))
            {
                data.GetNoRefs(noRefPath);
            }

            EditorGUILayout.EndHorizontal();
        }

        messPath = EditorPrefs.GetString(messPathPrefabKey);
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(messPath, (GUIStyle)"HelpBox");
            if (GUILayout.Button("报告导出路径"))
            {
                var res = EditorUtility.OpenFolderPanel("选择报告导出路径", messPath, messPath);
                if (string.IsNullOrEmpty(res))
                {
                    return;
                }
                messPath = res;
                EditorPrefs.SetString(messPathPrefabKey, messPath);
                Repaint();
                return;
            }

            if (GUILayout.Button("乱引用Assets"))
            {
                data.GetMassRefs(messPath);
            }
